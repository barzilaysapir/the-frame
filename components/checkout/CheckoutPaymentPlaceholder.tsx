...
import { confirmCheckoutSession } from "@/lib/client/confirm-checkout-session";
...
interface PurchaseApiResponse {
  ...
  /** Present once Stripe is configured — navigate the buyer here (Stripe Checkout is a GET redirect, not a POST form). */
  checkoutUrl?: string;
}
...
/**
 * Starts Stripe Checkout (see POST /api/v1/me/purchases and
 * lib/server/payments/stripe.ts) — the only payment gateway wired up.
...
export function CheckoutPaymentPlaceholder({
  ...
  }, [user, returnedFromPayment, returnedSessionId]);
