 ...
 * header). Real R2 keys use a presigned GET when credentials exist.
 * HMAC `/stream` is the fallback (and 302s to R2 when it can). Demo
 * `https://` sources always use `/stream`.
 *
